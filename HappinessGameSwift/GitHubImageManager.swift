...
class GitHubImageManager: ObservableObject {
    ...
    private func uploadImage(_ image: UIImage, fileName: String, type: String, completion: @escaping (Result<String, Error>) -> Void) {
        ...
        guard let url = URL(string: "https://api.github.com/repos/\(repo.owner)/\(repo.name)/contents/\(path)") else {
            completion(.failure(NSError(domain: "GitHubImageManager", code: 0, userInfo: [NSLocalizedDescriptionKey: "Invalid URL"])))
            return
        }
    ...
    private func updateImage(repo: GitHubRepository, imageData: Data, path: String, type: String = "visit-plan", completion: @escaping (Result<String, Error>) -> Void) {
        // まず現在のファイル情報を取得
        guard let getUrl = URL(string: "https://api.github.com/repos/\(repo.owner)/\(repo.name)/contents/\(path)") else {
            completion(.failure(NSError(domain: "GitHubImageManager", code: 0, userInfo: [NSLocalizedDescriptionKey: "Invalid URL for getting file info"])))
            return
        }
        ...
        URLSession.shared.dataTask(with: getRequest) { data, response, error in
            ...
            // ファイルを更新
            guard let url = URL(string: "https://api.github.com/repos/\(repo.owner)/\(repo.name)/contents/\(path)") else {
                completion(.failure(NSError(domain: "GitHubImageManager", code: 0, userInfo: [NSLocalizedDescriptionKey: "Invalid URL for updating file"])))
                return
            }
            var request = URLRequest(url: url)
            request.httpMethod = "PUT"
    ...
    private func deleteFromGitHub(repo: GitHubRepository, path: String, recordId: String, size: Int64, completion: @escaping (Result<Void, Error>) -> Void) {
        ...
        URLSession.shared.dataTask(with: getRequest) { data, response, error in
            ...
            // ファイルを削除
            guard let deleteUrl = URL(string: "https://api.github.com/repos/\(repo.owner)/\(repo.name)/contents/\(path)") else {
                completion(.failure(NSError(domain: "GitHubImageManager", code: 0, userInfo: [NSLocalizedDescriptionKey: "Invalid URL for deleting file"])))
                return
            }
    ...
    func checkRepositoryCapacity(completion: @escaping (Result<[GitHubRepository], Error>) -> Void) {
        ...
        for repo in settings.repositories {
            ...
            guard let url = URL(string: "https://api.github.com/repos/\(repo.owner)/\(repo.name)") else {
